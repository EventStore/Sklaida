...
.factory('searchService', ['$http', function searchService($http){
	return {
		search: function(data, callback){
			var headers = {'Content-Type' : 'application/vnd.ouroinc.searchrequest+json; charset=utf-8'};
			$http.post('http://localhost:9000/search', data, { headers : headers })
				.success(function(data, status, header, config){
					callback(null, {Location: header('Location')});
				});	
		},
		pollForSearchResult: function(searchResultUrlToPoll, callback){
			...
			var headers = {'Accept' : 'application/vnd.eventstore.atom+json'};

			var schedule = function(){
				setTimeout(function(){
					$http.get(locationToPoll, { headers : headers })
					.success(function(data, status, header, config){
						callback(null, data);
						schedule();
					});
